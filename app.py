...
# ============================================================
# DELIBERATELY INTRODUCED VULNERABILITIES FOR ASSIGNMENT 2
# ============================================================

# --- VULNERABILITY 1: For SonarQube Detection ---
# CWE-78: OS Command Injection via subprocess with shell=True
# SonarQube flags this as a Critical Security Hotspot (python:S2076)
import subprocess

@app.route('/api/ping', methods=['GET'])
def ping_host():
    host = request.args.get('host', 'localhost')
    # DELIBERATELY INSECURE: user input passed directly to shell
    result = subprocess.run(f"ping -c 1 {host}", shell=True, capture_output=True, text=True)
    return jsonify({'output': result.stdout})


# --- VULNERABILITY 2: For ZAP Detection ---
# CWE-79: Reflected Cross-Site Scripting (XSS)
# ZAP detects this via active scanning — user input echoed in HTML without sanitisation
from markupsafe import escape as _escape  # imported but NOT used below (intentional)

@app.route('/search', methods=['GET'])
def search():
    query = request.args.get('q', '')
    # DELIBERATELY INSECURE: user input rendered directly in HTML response
    return f"<html><body><h2>Search Results for: {query}</h2></body></html>", 200, {
